...
haarfile = cv2.data.haarcascades+"haarcascade_eye_tree_eyeglasses.xml"
eyedetector = cv2.CascadeClassifier(haarfile) #加载眼睛级联分类器
...
if not cam.isOpened():
    ...
    while True:
        ...
        for (x, y, w, h) in faces: #枚举所有脸
            ...
            eyes = eyedetector.detectMultiScale(gray[y:y+h, x:x+w],1.1, 1, cv2.CASCADE_SCALE_IMAGE, (2,2)) #检测眼睛
            for (ex, ey, ew, eh) in eyes: #枚举所有眼睛
                cv2.rectangle(image[y:y+h, x:x+w], (x+ex, y+ey), (x+ex+ew, y+ey+eh), (255, 0, 0), 1) #在眼睛四周画蓝框
